		...
		struct cxlmi_cmd_dcd_get_dc_config_rsp *ret)
{
	...
	for (i = 0; i < ret->regions_returned; i++) {
		...
		p += sizeof(uint64_t);
		ret->records[i].decode_len = le64_to_cpu(*(uint64_t *)p) * CXL_CAPACITY_MULTIPLIER;
		p += sizeof(uint64_t);
...
CXLMI_EXPORT int cxlmi_cmd_dcd_get_dc_extent_list(struct cxlmi_endpoint *ep,
		struct cxlmi_tunnel_info *ti,
		struct cxlmi_cmd_dcd_get_dc_extent_list_req *in,
		struct cxlmi_cmd_dcd_get_dc_extent_list_rsp *ret)
{
	struct cxlmi_cmd_dcd_get_dc_extent_list_req *req_pl;
	struct cxlmi_cmd_dcd_get_dc_extent_list_rsp *rsp_pl;
	_cleanup_free_ struct cxlmi_cci_msg *req = NULL;
	_cleanup_free_ struct cxlmi_cci_msg *rsp = NULL;
	ssize_t req_sz, rsp_sz, rsp_sz_min;
	int i, rc = -1;
	...
	req_pl = (struct cxlmi_cmd_dcd_get_dc_extent_list_req *)req->payload;
	...
	rsp_pl = (struct cxlmi_cmd_dcd_get_dc_extent_list_rsp *)rsp->payload;
	memset(ret, 0, sizeof(*ret));
...
CXLMI_EXPORT int cxlmi_cmd_dcd_add_dyn_cap_response(struct cxlmi_endpoint *ep,
		struct cxlmi_tunnel_info *ti,
