...
struct cxlmi_cmd_dcd_get_dc_config_req {
	uint8_t region_cnt;
	uint8_t start_region_id;
} __attribute__((packed));

struct cxlmi_cmd_dcd_get_dc_config_rsp {
    uint8_t num_regions;
 ...
 * (Opcode 4801h)
 */
struct cxlmi_cmd_dcd_get_dc_extent_list_req {
    uint32_t extent_cnt;
    uint32_t start_extent_idx;
} __attribute__((packed));

struct cxlmi_cmd_dcd_get_dc_extent_list_rsp {
 ...
 * (Opcode 4802h)
 */
struct cxlmi_cmd_dcd_updated_dyn_cap_req {
